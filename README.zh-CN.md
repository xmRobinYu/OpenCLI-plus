...
除了网站能力，OpenCLI 还是一个 **CLI 枢纽**：你可以把 `gh`、`docker`、`longbridge`、`bili(bilibili-cli)`、`lark-cli`、`zsxq-cli`、`tg`、`discord`、`wx`、`ntn`（Notion）等本地工具统一注册到 `opencli` 下，也可以通过桌面端适配器控制 Cursor、Trae CN、Codex、Antigravity、ChatGPT、Trae SOLO 等 Electron 应用。
...
- `opencli open <url>` 根据链接自动路由到最合适的内置命令
...
| **bilibili** | `hot` `search` `me` `status` `whoami` `user` `favorite` `favorites` `history` `history-bridge` `feed` `feed-bridge` `my-dynamics` `dynamic-post` `dynamic-delete` `watch-later` `following` `following-bridge` `subtitle` `summary` `video` `comments` `dynamic` `ranking` `user-videos` `download` |
...
| **知识星球** | 话题（Markdown） | 导出单条知识星球话题，包含评论和图片 |
| **飞书** | 文档页（Markdown） | 从已登录浏览器导出飞书 / Lark 文档或 Wiki 页面 |
